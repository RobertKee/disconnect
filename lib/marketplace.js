...
module.exports = function(client){
	...
	marketplace.getFee = function(price, currency, callback){
		...
		path = adjustPathForCurrency(path, currency)
	...
	/**
	 * Get the stats about a given release ID
	 * @param {(number|string)} release - The release ID
	 * @param {string} [currency] - Optional currency as one of USD, GBP, EUR, CAD, AUD, or JPY. Defaults to USD.
	 * @param {function} [callback] - The callback
	 * @return {DiscogsClient|Promise}
	...
	marketplace.getStats = function(release, currency, callback){
		var path = '/marketplace/stats/'+release + adjustPathForCurrency(currency)
		return client.get(path, callback)
	}

	function adjustPathForCurrency (path, currency) {
		if(typeof currency !== 'function'){ // Get the fee in a given currency
			path += '/'+currency;
		}
